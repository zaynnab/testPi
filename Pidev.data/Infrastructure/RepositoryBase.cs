...
namespace MyFinance.Data.Infrastructure
{
    public class RepositoryBase<T> : IRepositoryBase<T> where T:class
    {
        ...
        public IEnumerable<T> GetMany(Expression<Func<T, bool>> Condition = null, Expression<Func<T, bool>> OrderBy = null)
        {
            ...
            IQueryable<T> Query = DbSet; //type de retour de la base de données IQueryable (dbset)
            if (Condition != null)
        ...
        {
            DbSet.Attach(entity);//attacher l entite au dbset
            ctx.Entry(entity).State = EntityState.Modified;//pour forcer la modification de l entite
